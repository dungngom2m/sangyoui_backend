...
use App\Http\Resources\CounselingReserveSangyoiDetail2Resource;
...
use App\Mail\ReserveSangyoiUpdateEmail;
...
class CounselingReserveSangyoiController extends Controller
{
    /**
     * Create a new CounselingReservesSangyoiController instance.
     *
     * @return void
     */
    ...
    /**
     * Counseling reserve list of sangyoi.
        ...
        try {
            ...
            switch ($request->orderSortBy) {
                ...
                case 'overtime':
                    ...
                    break;
                case 'smsNumber':
                    $orderSortBy = 'm_client_employee.sms_number';
                    break;
                default:
                    $orderSortBy = 't_counseling_reserve.id';
            }
            ...
            $list = TCounselingReserve::join('t_counseling_reserve_notice', function ($join) {
                                    ...
                                    ->when($orderSortBy == 't_counseling_reserve.reserve_date', function ($q) use ($request) {
                                        return $q->orderBy('t_counseling_reserve.reserve_time_from', $request->orderBy);
                                    })
                                    ->where('t_counseling_reserve.reserve_status', '!=', '2')
                                    ->paginate($request->itemsPerPage ?? 10);

            return $this->response(200, [
                'counselingReserve' => new CounselingReserveSangyoiCollection($list)
    ...
    /**
     * Show counseling reserve sangyoi.
     * @param mixed $id
     * @return \Illuminate\Http\JsonResponse
     */
    public function show($id) {
        try {
            ...
            $notice = TCounselingReserve::join('t_counseling_reserve_notice', function ($join) {
                                            $join->on('t_counseling_reserve_notice.id', '=', 't_counseling_reserve.reserve_issue_id');
                                            $join->whereNull('t_counseling_reserve_notice.deleted_at');
                                        })
                                        ->where('t_counseling_reserve.id', $id)
                                        ->select(
            ...
            $countReserveOfDoctor = TCounselingReserveNotice::select(
                                            't_counseling_reserve_notice.client_id',
                                            't_counseling_reserve_notice.user_company_id',
                                            't_counseling_reserve_notice.employee_id',
                                        )
            ...
            $detail = TCounselingReserve::join('t_counseling_reserve_notice', function ($join) {
                                            $join->on('t_counseling_reserve_notice.id', '=', 't_counseling_reserve.reserve_issue_id');
                                            $join->whereNull('t_counseling_reserve_notice.deleted_at');
                                        })
            ...
            $detail->count_reserve_of_doctor = count($countReserveOfDoctor) ?? 0;

            $questionAnswers = TQuestionAnswer::where('reserve_issue_id', $detail->reserve_issue_id)
            ...
            $reportInfo = TCounselingReserve::leftJoin('t_report_hospital', function ($join) {
            ...
            $updateInfos = TReportHospital::join('t_counseling_reserve', function ($join) {
            ...
            $selfChecklist = $this->calculateScoreAnswer($detail->reserve_issue_id);

            return $this->response(200, [
                'counselingReserve' => new CounselingReserveSangyoiDetailResource($detail),
                'questionAnswers' => new QuestionAnswerCollection($questionAnswers),
                'report' => new CounselingReserveSangyoiDetail2Resource($reportInfo),
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $reserve = TCounselingReserve::join('t_counseling_reserve_notice', function ($join) {
                                            $join->on('t_counseling_reserve_notice.id', '=', 't_counseling_reserve.reserve_issue_id');
                                            $join->whereNull('t_counseling_reserve_notice.deleted_at');
                                        })
                                        ->join('m_client_employee', function ($join) {
                                        ...
                                        ->select(
                                            ...
                                            't_counseling_reserve.reserve_issue_id as reserveIssueId'
                                        )
                                        ->first();

            $reportQuery = TCounselingReserve::join('t_report_hospital', function ($join) {
                                                $join->on('t_report_hospital.reserve_issue_id', '=', 't_counseling_reserve.reserve_issue_id');
                                                $join->whereNull('t_report_hospital.deleted_at');
                                            })
                                            ->where('t_counseling_reserve.id', $id)
            ...
            $notice = TCounselingReserveNotice::find($reserve->reserveIssueId);
            $notice->implementation_status = 3;
            $notice->save();
            ...
            if (isset($reportQuery->id)) {
                ...
                $report->reserve_issue_id = $notice->id;
                $report->suimin_situation = $request->suiminSituation;
                $report->suimin_situation_memo = $request->suiminSituationMemo;
                $report->hirou_situation = $request->hirouSituation;
                $report->hirou_situation_memo = $request->hirouSituationMemo;
            ...
            $dateParam = new Carbon($reserve->reserveDate);
            $timeParam = new Carbon($reserve->reserveTimeFrom);

            $users = MUser::where('user_company_id', $reserve->userCompanyId)->get();

            $url = sprintf('%s/cp/user/hospital/?id=%s', env('FRONT_URL') ?: $_SERVER['FRONT_URL'], $report->reserve_issue_id);

            foreach($users as $user) {
                Mail::to($user->mailaddr)->send(new ReserveSangyoiUpdateEmail([
                    'url' => $url,
                    'employeeName' => $reserve->employeeName,
                    'sangyoiName' => $reserve->sangyoiName,
